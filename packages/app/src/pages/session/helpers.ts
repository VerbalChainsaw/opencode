...
export const createSessionTabs = (input: TabsInput) => {
  ...
  const openedTabs = createMemo(
    () => {
      ...
      return input
        ...
        .all()
        .flatMap((tab) => {
          if (tab === "context" || tab === "review" || tab === "goal") return []
  ...
  const activeTab = createMemo(() => {
    ...
    // "goal" is a fixed trigger like review/context; the side panel
    // closes the tab when the goal disappears, so no visibility guard
    // is needed here.
    if (active === "goal") return active
