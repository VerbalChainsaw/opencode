...
test("envelope: chain remove deletes a pending one-based step and updates chainTotal", () => {
  ...
  try {
    ...
    const remove = dispatchGoalCommandStructured(dir, "chain remove 2");
    assert.equal(remove.kind, "success", remove.message);
    assert.match(remove.message, /Step 2 removed/);

    const chain = JSON.parse(readFileSync(join(dir, ".opencode", ".goal-chain.json"), "utf-8"));
    assert.deepEqual(chain.steps.map((step) => step.condition), ["Plan", "Document"]);
...
test("envelope: chain move accepts user-facing one-based step numbers", () => {
  const dir = freshDir();
  try {
    const payload = JSON.stringify({
      steps: [
    ...
    const move = dispatchGoalCommandStructured(dir, "chain move 1 3");
    assert.equal(move.kind, "success", move.message);

    const chain = JSON.parse(readFileSync(join(dir, ".opencode", ".goal-chain.json"), "utf-8"));
    assert.deepEqual(chain.steps.map((step) => step.condition), ["Build", "Document", "Plan"]);
    assert.equal(chain.current, 2);

    const state = JSON.parse(readFileSync(join(dir, ".opencode", ".goal-state.json"), "utf-8"));
    assert.equal(state.condition, "Plan");
