...
describe("goal panel mission-control contracts", () => {
  ...
  test("runtime detail actions share one stable tray slot instead of mounting page sections", async () => {
    ...
    expect(src).toContain('class="order-first mb-1.5 rounded-lg border p-2"')
    ...
    expect(src).toContain('data-component="goal-runtime-actions"')
    expect(src).toContain('class="grid min-w-0 grid-cols-[repeat(auto-fit,minmax(116px,1fr))] gap-1.5"')
    expect(src).toContain('class="min-h-8 w-full min-w-0 px-2 text-center text-11-medium leading-tight"')
    expect(src).toContain("<Show when={confirmingClear() || steerOpen() || handoffOpen()}>")
    expect(src).toContain('class="grid min-h-9 min-w-0 grid-cols-[minmax(0,1fr)_minmax(124px,1fr)] gap-1.5 rounded-md border px-2 py-1.5"')
    expect(src).toContain('data-component="goal-runtime-steer-field" class="[grid-column:1/-1] min-w-0"')
    expect(src).toContain('data-component="goal-runtime-handoff-field" class="[grid-column:1/-1] min-w-0"')
    expect(src).toContain('class="w-full min-w-0"')
    ...
    expect(src).not.toContain('data-component="goal-runtime-primary-actions"')
    expect(src).not.toContain('data-component="goal-runtime-support-actions"')
    expect(src).not.toContain('class="h-7 w-full min-w-0 px-2 text-11-medium"')
    expect(src).not.toContain("session.goal.commandStrip.hint")
  ...
  test("chain target field does not expose raw template variables in the operator UI", async () => {
    const src = await goalPanelSource()
    const fieldStart = src.indexOf('data-component="goal-chain-target-field"')
    const fieldEnd = src.indexOf("<Show when={!hasRunnableChain()}>", fieldStart)
    expect(fieldStart).toBeGreaterThan(-1)
    expect(fieldEnd).toBeGreaterThan(fieldStart)

    const field = src.slice(fieldStart, fieldEnd)
    expect(field).toContain('title={language.t("session.goal.chainBuilder.objectiveHint")}')
    expect(field).not.toContain("grid-cols-[auto_minmax(0,1fr)_auto]")
    expect(field).not.toContain('{"{scope}"}')
    expect(field).not.toContain("{scope}")

    const localeFiles = await Promise.all([
      sourceText("../../i18n/en.ts"),
      sourceText("../../i18n/zh.ts"),
      sourceText("../../i18n/zht.ts"),
    ])
    for (const localeFile of localeFiles) {
      expect(localeFile).not.toMatch(/"session\.goal\.chainBuilder\.objectiveHint":.*\{scope\}/)
    }
  })
  ...
  test("chain builder header is a compact control strip instead of tall budget cards", async () => {
    ...
    expect(src).toContain("grid-cols-[repeat(auto-fit,minmax(150px,1fr))]")
    expect(src).not.toContain("sm:grid-cols-3")
    expect(src).not.toContain("lg:grid-cols-[minmax(260px,1fr)_auto]")
  ...
  test("run-order rows show runtime metadata and direct draft-row editing", async () => {
    ...
    expect(src).toContain("chainStepRuntimeLabels")
  ...
  test("playbook workspace matches the approved session-native layout", async () => {
    ...
    expect(src).toContain("overflow-hidden rounded-md border px-1 py-0.5")
    expect(src).not.toContain("overflow-hidden rounded border px-1 py-0.5")
    expect(src).not.toContain("rounded border")
  ...
  test("playbook visual pass keeps chain rows dense and library chrome explicit", async () => {
    ...
    expect(src).not.toContain("min-w-[540px]")
    expect(src).not.toContain("min-w-[640px]")
    expect(src).not.toContain("grid-cols-[24px_32px_minmax(96px,1fr)_minmax(56px,68px)_minmax(128px,148px)_96px]")
    ...
    expect(src).toContain('class="flex min-w-0 flex-wrap items-center justify-between gap-2"')
    expect(src).toContain('class="flex min-w-[160px] flex-1 flex-wrap items-center gap-1.5"')
    expect(src).toContain('class="flex min-w-[160px] flex-1 flex-wrap items-center justify-end gap-1.5"')
    expect(src).toContain('class="min-w-0 flex-1 basis-[120px] truncate text-[11px] font-semibold text-text-weak"')
    expect(src).toContain("grid-cols-[repeat(auto-fit,minmax(150px,1fr))]")
    expect(src).not.toContain("grid-cols-1 gap-1.5 sm:grid-cols-3")
  ...
  test("live goals keep the chain builder as the running screen and highlight steps", async () => {
    ...
    expect(src).toContain('data-component="goal-running-now-step-runtime"')
    expect(src).toContain("session.goal.chainBuilder.currentStep")
    expect(src).toContain("session.goal.chainBuilder.currentStepAria")
    expect(src).toContain("session.goal.runtime.agent")
    expect(src).toContain("session.goal.runtime.model")
    expect(src).toContain("session.goal.runtime.skills")
    expect(src).toContain("session.goal.runtime.completion")
    ...
    expect(src).toContain("flex min-w-[160px] flex-1 flex-wrap items-center gap-1.5")
    expect(src).toContain("min-w-0 flex-1 basis-[120px] truncate")
    expect(src).not.toContain("lg:grid-cols-[minmax(260px,1fr)_auto]")
    expect(src).toContain("flex min-w-0 flex-wrap items-center justify-between")
  ...
  test("goal console uses high-contrast command buttons and meaningful helper copy", async () => {
    ...
    expect(src).toContain("inline-flex min-w-0 items-center justify-center truncate rounded-md border px-2 py-0 text-11-medium font-semibold leading-none")
    expect(src).not.toContain("inline-flex min-w-0 items-center justify-center truncate rounded-md border px-2 py-1")
    ...
    expect(src).toContain("grid min-w-0 grid-cols-[repeat(auto-fit,minmax(116px,1fr))] gap-1.5")
    expect(src).toContain("grid-cols-[minmax(0,1fr)_minmax(124px,1fr)]")
    expect(src).toContain('data-component="goal-runtime-steer-field"')
    expect(src).toContain('data-component="goal-runtime-handoff-field"')
  ...
  test("button scoping keeps local edits, template mutations, and run boundaries separate", async () => {
    ...
    expect(createGoal).toContain("goalControlQuotedArg(condition)")
    expect(createGoal).toContain("goalControlQuotedArg(command)")
    expect(createGoal).not.toContain('replace(/"/g')
    ...
    expect(steerGoal).toContain('sendGoalCommand("steer", `steer ${goalControlQuotedArg(note)}`)')
    expect(steerGoal).not.toContain('replace(/"/g')
    ...
    expect(handoffGoal).toContain("goalControlQuotedArg(note)")
    expect(handoffGoal).not.toContain('replace(/"/g')
  ...
  test("main goal form starts a goal when no chain items exist", async () => {
    ...
    expect(src).toContain("const condition = chainDraft.objective.trim()")
    expect(src).toContain("goalControlQuotedArg(condition)")
    expect(src).not.toContain('chainDraft.objective.trim().replace(/"/g')
    expect(src).toContain('data-component="goal-chain-target-field"')
    expect(src).toContain('data-component="goal-standalone-command-field"')
  ...
  test("chain row actions wrap metadata instead of shrinking buttons", async () => {
    const src = await goalPanelSource()
    const rowStart = src.indexOf('data-component="goal-chain-step-row"')
    const rowEnd = src.indexOf('data-component="goal-chain-summary-line"', rowStart)
    expect(rowStart).toBeGreaterThan(-1)
    expect(rowEnd).toBeGreaterThan(rowStart)

    const row = src.slice(rowStart, rowEnd)
    expect(row).toContain("grid-cols-[24px_32px_minmax(0,1fr)]")
    expect(row).toContain('data-component="goal-chain-step-meta"')
    expect(row).toContain('data-component="goal-chain-step-actions" class="flex min-w-[108px] shrink-0')
    expect(row).not.toContain("grid-cols-[24px_32px_minmax(96px,1fr)_minmax(56px,68px)_minmax(128px,148px)_96px]")

    const inlineButtonStart = src.indexOf("function inlineCommandButtonClass")
    const inlineButtonEnd = src.indexOf("function inlineCommandButtonStyle", inlineButtonStart)
    expect(inlineButtonStart).toBeGreaterThan(-1)
    expect(inlineButtonEnd).toBeGreaterThan(inlineButtonStart)
    const inlineButton = src.slice(inlineButtonStart, inlineButtonEnd)
    expect(inlineButton).toContain("h-6 min-w-6 shrink-0")
  })
...
describe("readGoalFromSdk", () => {
  ...
  test("duplicate JSON keys resolve to last value and validate normally", async () => {
    const duped =
      '{"status":"active","status":"cleared","id":"x","condition":"y","startedAt":0,"completedAt":null,"turnsEvaluated":0,"tokensUsed":0,"constraints":{"maxTurns":5,"maxTimeMinutes":10,"maxTokens":1000}}'
