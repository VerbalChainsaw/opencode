...
describe("validateTemplate", () => {
  ...
  test("accepts desktop action metadata including skills and a structured model pin", () => {
    assert.equal(validateTemplate({
  ...
  test("rejects malformed desktop action metadata", () => {
    assert.equal(validateTemplate({ condition: "x", skills: ["ok", "ok"] }), false);
    assert.equal(validateTemplate({ condition: "x", model: { providerID: "openai" } }), false);
  });
