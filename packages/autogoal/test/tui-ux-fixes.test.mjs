...
test("tui.tsx: the active/paused view also has an Esc hint (user is not stuck with a goal set)", () => {
  ...
  // Count <DashboardFooter> usages — must be exactly 3 (no-goal + active/paused
  // + corrupt). The v0.7.0 audit added the corrupt branch so the dashboard
  // surfaces a recoverable error when the state file fails validation;
  // without <DashboardFooter> on that branch, users hitting a corrupt
  // state file would have no Esc affordance. If a future branch is added,
  // update this count intentionally, not silently.
  const usageCount = (tuiSrc.match(/<DashboardFooter\b/g) ?? []).length;
  assert.equal(usageCount, 3,
    `expected exactly 3 <DashboardFooter> usages (no-goal fallback + active/paused view + corrupt branch), found: ${usageCount}`);
});
