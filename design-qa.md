...
- Electron screenshot after current-step runtime context fix: `C:\Users\zerop\Development\opencode-source\output\playwright\desktop-goal-current-step-runtime-20260627.png`
...
- Home metric context defect fixed in this pass: live Electron Home showed zero-count cards with active-state labels (`Live Sessions 0 working now`, `Active Goals 0 running`). The metric strip now uses count-aware detail copy; post-fix Electron measured `Live Sessions 0 idle` and `Active Goals 0 none running`, with all metric cards 193x92 and `bad: []`.
- Current-step runtime context defect fixed in this pass: the running-chain current-step strip previously showed the active step title and index but not the runtime route, forcing operators to infer agent/model/skills/completion rule from editor state. It now renders wrapped runtime chips inside the current-step strip. A live Electron active-chain fixture measured `Agent build`, `Model OpenAI / GPT-5.4`, `Skills 2 skills`, and `Completion Completes when the agent reports completion` inside a 291.5x40 runtime region with `badVisibleControls: []`; the fixture restored the original `.opencode/.goal-state.json` and `.opencode/.goal-chain.json` afterward.
- Goal radius consistency defect fixed in this pass: remaining oversized `rounded-xl`/`rounded-2xl` surfaces in the Goal shortcut overlay and recent-run history section were normalized to the same 8px-or-less console radius system. Live Electron measured the shortcut overlay at `radius: 8px`, 288x176, and `overflowsViewport: false`; the Goal panel source guard now rejects `rounded-xl` and `rounded-2xl`.
- Action editor instruction fit defect fixed in this pass: live Electron visible-control scan found the reusable-action `Instruction` textarea internally clipped (`scrollHeight 75 > clientHeight 54`). The editor prompt field now has a local 76px floor; post-fix Electron measured 102 visible Goal controls with `bad: []`, and the `Instruction` textarea at 362x76 with `scrollHeight === clientHeight`.
