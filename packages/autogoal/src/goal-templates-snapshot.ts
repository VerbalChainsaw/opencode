...
export interface TemplateButton {
  ...
  /** Optional Desktop action metadata. */
  skills?: GoalTemplate["skills"];
  model?: GoalTemplate["model"];
  category?: GoalTemplate["category"];
  tone?: GoalTemplate["tone"];
  elevation?: GoalTemplate["elevation"];
...
export function buildTemplatesSnapshot(directory: string): TemplatesSnapshot {
  ...
  for (const t of discovered) {
    ...
    byId.set(t.name, {
      ...
      ...(tpl.skills ? { skills: tpl.skills } : {}),
      ...(tpl.model ? { model: tpl.model } : {}),
      ...(tpl.category ? { category: tpl.category } : {}),
      ...(tpl.tone ? { tone: tpl.tone } : {}),
      ...(tpl.elevation ? { elevation: tpl.elevation } : {}),
      builtin: t.builtin,
    });
  }
  return {
