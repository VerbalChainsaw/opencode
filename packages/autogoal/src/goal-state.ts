...
//
// v0.7.0 — added an IN-PROCESS JS mutex (`withStateLock`) for the multi-
// async-window race identified in the v0.7.0 audit (Call 2 R1, R5). The
// tool handlers `clear_goal` / `pause_goal` / `resume_goal` / `goal_webhook`
// / `restart_goal` and the auto-loop's `evaluate()` IIFE all mutate the
// state file. They were never coordinated — concurrent invocations could
// clobber each other's field updates ("last rename wins", bounded-impact).
// `withStateLock` serializes them per-directory in a single Node process.
// Inter-process coordination is intentionally out of scope (each server
// plugin instance has its own state-mutation paths; cross-instance is the
// same last-rename-wins surface that existed pre-fix).

/**
 * Per-directory in-process JS mutex.
 *
 * Serializes state-mutating callbacks within a single Node process so the
 * tool handlers and the auto-loop's evaluate() IIFE don't clobber each
 * other's field updates. Per-directory (multiple workspaces in one process
 * run in parallel). The mutex slot is always cleared, even if `fn` throws,
 * so a panic in one callback doesn't deadlock subsequent callers.
 *
 * Inter-process coordination is intentionally out of scope: each server
 * plugin instance has its own mutation paths and the same "last rename
 * wins" inter-process surface that the v0.4.0 architecture leaves in place.
 *
 * @param directory - the workspace directory; locks are keyed on this
 * @param fn - the callback to run while holding the lock. May be sync or
 *             async; the return value is awaited and returned.
 */
export async function withStateLock<T>(directory: string, fn: () => Promise<T> | T): Promise<T> {
  const prev = stateLocks.get(directory) ?? Promise.resolve();
  // Chain `fn` onto the prior promise. Catch+swallow on the stored slot
  // so a throw from `fn` doesn't poison the chain for the next caller.
  const next = prev.then(() => fn(), () => fn());
  stateLocks.set(directory, next.catch(() => {}));
  return next;
}
...
/**
 * Atomically toggle an active goal between paused and active.
 *
 * atomicToggle is fully synchronous; Node's single-threaded event loop
 * means the read-modify-write inside this function cannot interleave
 * with another invocation. The atomic-write primitive (tmp + rename)
 * makes last-rename-wins safe under concurrent invocations from
 * different call paths. IIFE guards in callers (id-equality check
 * after read-back) defend against the case where the state id changed
 * between the caller's snapshot and this function's write.
 *
