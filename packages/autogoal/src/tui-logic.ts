...
export type { GoalState, ReadResult };
...
export interface DashboardView {
  /** Active or paused; null means "no goal to display" (covers both absent
   *  and corrupt — check `readResult.kind` to distinguish). */
  state: GoalState | null;
  /** Tri-state read result. `kind: "corrupt"` means the on-disk state file
   *  failed validation; the corrupt copy has been quarantined as
   *  `.goal-state.json.corrupt.<ts>` by `readGoalStateResult`, and the user
   *  can run `/goal clear` to start fresh. v0.7.0 audit fix. */
  readResult: ReadResult<GoalState>;
}

export function readDashboardState(directory: string): DashboardView {
  const result = readGoalStateResult(directory);
  if (result.kind === "ok") return { state: result.value, readResult: result };
  // Both "absent" and "corrupt" collapse to `state: null` for backward
  // compat with consumers that only check `.state`. New consumers can
  // branch on `readResult.kind` to show a distinct ERROR render.
  return { state: null, readResult: result };
}
