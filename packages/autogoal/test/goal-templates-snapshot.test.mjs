...
test("buildTemplatesSnapshot: surfaces a valid user template as a non-builtin button", () => {
  ...
  assert.deepEqual(user.skills, ["frontend-design", "playwright"]);
  assert.deepEqual(user.model, { providerID: "openai", modelID: "gpt-5" });
  assert.equal(user.category, "Building");
  assert.equal(user.tone, "emerald");
  assert.equal(user.elevation, "raised");
});
