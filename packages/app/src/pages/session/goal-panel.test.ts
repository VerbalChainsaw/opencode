...
describe("goal panel mission-control contracts", () => {
  ...
  test("goal console uses the four-zone visual architecture from the target mock", async () => {
    ...
    expect(src).toContain("text-[15px] font-extrabold uppercase leading-5 text-white [letter-spacing:0]")
  ...
  test("action library rows are compact command rows instead of tall category cards", async () => {
    ...
    expect(librarySrc).toContain("grid min-h-0 grid-cols-[minmax(0,1fr)_26px_26px]")
  ...
  test("goal console uses high-contrast command buttons and meaningful helper copy", async () => {
    ...
    expect(src).toContain('"background-color": "rgba(16, 185, 129, 0.10)"')
    expect(src).toContain("function actionEditorPanelStyle")
    expect(src).toContain("rgba(251, 191, 36, 0.46)")
    expect(src).toContain('"background-color": "rgba(245, 158, 11, 0.12)"')
    expect(src).toContain('"border-color": "rgba(252, 211, 77, 0.34)"')
    expect(src).toContain("border bg-background-panel/80")
    expect(src).toContain("border-b border-border-base/70 px-3 py-3")
    expect(src).toContain("h-6 w-2 shrink-0 rounded-sm")
    ...
    expect(src).toContain("Save changes. Add from Action Library inserts into Run Order.")
