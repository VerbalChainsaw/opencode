...
function currentChainStepPinnedModel(directory: string): GoalPinnedModel | null {
  ...
  // String form: "providerID:modelID" or "providerID/modelID"
  if (typeof raw === "string") {
    const trimmed = raw.trim();
    if (!trimmed) return null;
    // Accept both colon (Desktop UI) and slash (CLI) separators
    const sep = Math.max(
      trimmed.indexOf(":"),
      trimmed.indexOf("/"),
    );
    if (sep > 0 && sep < trimmed.length - 1) {
      const providerID = sanitizeForPrompt(trimmed.slice(0, sep)).trim();
      const modelID = sanitizeForPrompt(trimmed.slice(sep + 1)).trim();
      if (providerID && modelID) return { providerID, modelID };
    }
    return null;
  }
  ...
  return {
    ...
    tool: {
      ...
      clear_goal: tool({
        ...
        args: {},
        async execute(_args, ctx) {
          // v0.7.0 (audit fix R1): wrap the read-mutate-write in
          // `withStateLock` so a concurrent `evaluate()` IIFE in the
          // auto-loop can't clobber field updates. Without the lock,
          // both paths read fresh, both decide, both write — last
          // rename wins, and the user sees a stale state. (Bounded
          // impact pre-fix; eliminated with the lock.)
          return await withStateLock(ctx.directory, () => {
      ...
      pause_goal: tool({
        ...
        args: {},
        async execute(_args, ctx) {
          // v0.7.0 (audit fix R1): wrap in withStateLock — see clear_goal.
          return await withStateLock(ctx.directory, () => {
            // Capture the pre-transition status so the webhook payload
      ...
      goal_webhook: tool({
        ...
        async execute(args, ctx) {
          // v0.7.0 (audit fix R5): wrap the read-then-write in withStateLock
          // so an auto-loop `evaluate()` field update between our read and
          // our write isn't clobbered. Without the lock, e.g. an
          // `appendSteering` racing with `goal_webhook` could lose the
    ...
    event: async ({ event }) => {
      ...
      switch (event.type) {
        ...
        case "session.error": {
          ...
          // v0.7.0 (audit fix): wrap in withStateLock so this
          // event-driven write doesn't race with concurrent tool handlers.
