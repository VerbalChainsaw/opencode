...
function useGoalState(api: TuiPluginApi, directory: string) {
  // v0.7.0: signal tracks the full DashboardView (state + readResult) so the
  // renderer can distinguish corrupt from absent. Previously only the state
  // itself was tracked, collapsing both into null (data loss invisible).
  const [view, setState] = createSignal<DashboardView>({
    state: null,
    readResult: { kind: "absent" },
  });
  ...
  const refresh = () => {
    ...
    queueMicrotask(() => {
      pending = false;
      setState(readDashboardState(directory));
    });
  };
  refresh();
...
function DashboardView(props: { api: TuiPluginApi; directory: string }) {
  const view = useGoalState(props.api, props.directory);
  const theme = () => props.api.theme.current;

  // v0.7.0: branch on readResult.kind so a corrupt state file renders an
  // ERROR view instead of being silently shown as "no goal" (the previous
  // behavior, which made data loss invisible to the user).
  const readResult = () => view().readResult;
  ...
  return (
    <box flexGrow={1} flexDirection="column" backgroundColor={theme().backgroundPanel} padding={1} gap={1}>
      ...
      <Show
        when={readResult().kind === "ok" && state() ? state() : null}
        fallback={
          // Read-result branches — absent and corrupt both render their
          // own flexGrow=1 box so Yoga has a measurable child and the
          // parent does not collapse to a black screen. The DashboardFooter
          // (rendered inside each branch) carries the Esc affordance.
          // v0.7.0 audit fix: corrupt branch is new; previously both
          // absent and corrupt collapsed into the "no goal" empty state,
          // making corrupt data loss invisible to the user.
          readResult().kind === "corrupt" ? (
            <box flexGrow={1} flexDirection="column" backgroundColor={theme().backgroundPanel} padding={1} gap={1}>
              <text fg={theme().error}>⚠ Goal state file is corrupt</text>
              {(() => {
                const corrupt = readResult() as Extract<ReadResult<GoalState>, { kind: "corrupt" }>;
                return (
                  <>
                    <text fg={theme().textMuted}>Reason: {corrupt.reason}</text>
                    <text fg={theme().textMuted}>Quarantined as: .opencode/.goal-state.json.corrupt.*</text>
                  </>
                );
              })()}
              <text fg={theme().text}>Run /goal clear to start fresh.</text>
              <DashboardFooter theme={theme} />
            </box>
          ) : (
            // absent — the original "no active goal" empty state.
            <box flexGrow={1} flexDirection="column" backgroundColor={theme().backgroundPanel} padding={1} gap={1}>
