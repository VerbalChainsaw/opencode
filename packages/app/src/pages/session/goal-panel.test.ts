...
describe("goal panel mission-control contracts", () => {
  ...
  test("runtime detail actions share one stable tray slot instead of mounting page sections", async () => {
    ...
    expect(src).toContain('class="grid min-w-0 grid-cols-2 gap-1.5"')
  ...
  test("playbook visual pass keeps chain rows dense and library chrome explicit", async () => {
    ...
    expect(src).toContain("grid-cols-[24px_32px_minmax(74px,1fr)_minmax(56px,68px)_minmax(88px,108px)_104px]")
    ...
    expect(src).toContain("grid-cols-[30px_minmax(24px,1fr)]")
    expect(src).toContain("grid-cols-[20px_minmax(28px,1fr)]")
    expect(src).toContain("flex min-w-[104px] shrink-0 items-center justify-end gap-1")
  ...
  test("goal console uses high-contrast command buttons and meaningful helper copy", async () => {
    ...
    expect(src).toContain("grid min-w-0 grid-cols-2 gap-1.5")
