...
export function dispatchGoalCommandStructured(
  ...
  if (action === "chain") {
    ...
    if (subAction === "move") {
      ...
      if (!m) return { kind: "usage", message: "Usage: /goal chain move <from-step> <to-step>" };
    ...
    if (subAction === "start") {
      ...
      try {
        ...
        // Path-traversal guard: chain files must live inside the workspace.
        // Mirrors the defense in server.ts:evaluateFile — without this, a
        // prompt-injected user could point the loader at any JSON file the
        // plugin process can read and have it silently become a chain.
        // (v0.7.0 audit fix — Call 4 finding.)
        const rel = relative(directory, chainPath);
        if (rel.startsWith("..") || isAbsolute(rel)) {
          return { kind: "invalid-value", message: "Chain file must be inside the workspace directory." };
        }
  ...
  return { kind: "unknown-action", message: 'Unknown /goal action. Try: set "<condition>", view, pause, resume, clear, template <name>, history, turns <n>, time <n>, tokens <n>, condition "<text>", steer "<hint>", unsteer, restart, handoff [note], claim, chain [skip|reset|start|remove].' };
}
